...
func (h *OAuthHandler) Callback(w http.ResponseWriter, r *http.Request) {
	...
	if req.Code == "" || req.RedirectURI == "" {
		jsonError(w, "code and redirect_uri are required", http.StatusBadRequest)
		return
	}
	...
	if err := json.Unmarshal(body, &tokenResp); err != nil {
		jsonError(w, "invalid token response", http.StatusBadGateway)
		return
	}

	jsonOK(w, tokenResp, http.StatusOK)
}
